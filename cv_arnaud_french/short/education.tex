...
\begin{cventries}
  ...
  {Oct. 2014 -- Nov. 2018} % Date(s)
  {
    ...
    \item \entrypositionstyle{Publications}: 3 publications en premier auteur -- finaliste au prix du meilleur
      article à SII 2016.
    \item \entrypositionstyle{Contributions}: Intégration d'algorithmes de \keyword{SLAM} au contrôle de
      robot humanoïdes, méthodes de recalage entre CAO et nuage de points (\keyword{ICP}), intégration de
      mesures de pose (\keyword{SLAM}), force et inertielles (\keyword{IMU}) dans un algorithme de marche par
      commande prédictive (\keyword{MPC}).
